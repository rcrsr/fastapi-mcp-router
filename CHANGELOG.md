...
- **BREAKING:** Pagination on `tools/list`, `resources/list`, `prompts/list`, and `resources/templates/list` caps each page at 100 items regardless of the negotiated protocol version. Existing 0.3.1 clients that negotiate `2025-03-26` or `2025-06-18` and don't send `cursor` now silently receive only the first 100 items instead of the full list, because those revisions predate `nextCursor`; such clients must be updated to follow `nextCursor` to retrieve items beyond the first page. ([#12](https://github.com/rcrsr/fastapi-mcp-router/pull/12))
- **BREAKING:** `ServerInfo.icons` entries passed to `create_mcp_router()` are now validated against the same `Icon` scheme/MIME allowlist as tool/resource/prompt icons (`https://` or `data:` sources; a 6-entry image MIME allowlist). Previously `ServerIcon` was an unvalidated `TypedDict` with no scheme or MIME restriction. A previously-accepted `http://` source or non-allowlisted MIME type now raises `-32602` on `initialize`. ([#12](https://github.com/rcrsr/fastapi-mcp-router/pull/12))
...
- **Dependency and CI maintenance:** Lowered fastapi and pydantic minimums, added lowest-direct CI job, SHA-pinned GitHub Actions, and added a PR area labeler with a label taxonomy. ([#15](https://github.com/rcrsr/fastapi-mcp-router/pull/15))
